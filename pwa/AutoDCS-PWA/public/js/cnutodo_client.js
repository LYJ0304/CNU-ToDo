    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // 알림 시간 저장 버튼 이벤트 리스너 수정
      const saveAlarmTimeButton = document.getElementById('saveAlarmTimeButton');
      if (saveAlarmTimeButton) {
        saveAlarmTimeButton.addEventListener('click', async () => {
          const dailyAlarmTime = document.getElementById('dailyAlarmTime').value;
          localStorage.setItem('dailyAlarmTime', dailyAlarmTime);
          logUserAction('alarm_time_saved', { time: dailyAlarmTime });

          if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
            showSnackbar('이 브라우저는 푸시 알림을 지원하지 않습니다.');
            return;
          }
          ...
          try {
            ...
            if (permission === 'default') {
              permission = await Notification.requestPermission(); // 권한 명시적 요청
            }

            if (permission === 'granted') {
              let subscription = await registration.pushManager.getSubscription();
              if (!subscription) {
                const newSubscription = await subscribeUser(registration); // 이미 granted 상태로 가정하고 호출
                if (newSubscription) {
                  showSnackbar('알림 구독이 완료되었고, 시간이 저장되었습니다.');
                  const studentNo = localStorage.getItem('studentno');
                  if (studentNo) {
                    fetchStudentInfo(studentNo); // 알림 재등록
                  }
                } else {
                  // subscribeUser 내부에서 구독 실패 시 null 반환, 적절한 스낵바는 subscribeUser가 표시할 수 있음
                  // 또는 여기서 "구독 설정에 실패했습니다."와 같은 메시지 추가 가능
                  showSnackbar('알림 구독 설정에 실패했습니다. 다시 시도해주세요.');
                }
              } else {
                showSnackbar('알림 시간이 저장되었습니다. 기존 구독을 사용합니다.');
                const studentNo = localStorage.getItem('studentno');
                if (studentNo) {
                  fetchStudentInfo(studentNo); // 알림 재등록
                }
              ...
              showSnackbar('알림 권한이 차단되었습니다. 브라우저 설정에서 변경해주세요.');
            } else { // 'default'인데 사용자가 프롬프트에 응답하지 않은 경우 등
              showSnackbar('알림을 받으려면 알림 권한을 허용해야 합니다.');
            }
            
            await updateNotificationStatus();
          } catch (error) {
            console.error("알림 시간 저장 또는 구독 중 오류:", error);
            showSnackbar("알림 처리 중 오류가 발생했습니다.");
            await updateNotificationStatus();
          }
        });
      }
  ...
  if (viewMode !== "time") {
    ...
    document.getElementById("saveStudentNoButton").addEventListener("click", async function() {
      ...
      try {
        ...
        if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
          showSnackbar('이 브라우저는 푸시 알림을 지원하지 않습니다. 학번만 저장됩니다.');
        } else {
          let permission = Notification.permission;
          if (permission === 'default') {
          ...
          if (permission === 'granted') {
            ...
            if (!subscription) {
                showSnackbar('알림 구독 설정에 실패했습니다. 학번은 저장되었지만, 알림은 동작하지 않을 수 있습니다.');
            }
          } else if (permission === 'denied') {
            showSnackbar('알림 권한이 차단되어 푸시 알림을 설정할 수 없습니다. 학번만 저장됩니다.');
          } else {
            showSnackbar('알림 권한이 허용되지 않아 푸시 알림을 설정할 수 없습니다. 학번만 저장됩니다.');
          }
        }
        ...
        console.error("학번 저장 또는 구독 과정에서 오류 발생:", error);
        showSnackbar("학번 저장 중 오류가 발생했습니다.");
        window.location.hash = "#main";
        fetchStudentInfo(studentNo); // 오류가 발생해도 학번으로 정보는 가져오도록 시도
      }
    });
      ...
      if (todoListChanged || alarmTimeChanged) {
        ...
        navigator.serviceWorker.ready.then(registration => {
          registration.pushManager.getSubscription().then(subscription => {
            if (!subscription) return; // 구독이 없으면 아무것도 안함 (이 시점에서는 이미 구독되어 있어야 함)
...
async function subscribeUser(registration) {
   ...
   // 이 함수는 Notification.permission === 'granted' 상태에서 호출된다고 가정합니다.
   if (Notification.permission !== 'granted') {
     // 혹시라도 권한이 granted가 아니면 구독 시도하지 않고 null 반환
     console.warn('subscribeUser 호출 시 알림 권한이 granted 상태가 아닙니다.');
     showSnackbar("알림 권한이 허용되지 않아 구독할 수 없습니다."); 
     updateNotificationStatus(); // UI 상태 업데이트
     return null;
   }
   ...
   try {
     ...
     // showSnackbar("푸시 알림이 활성화되었습니다."); // 호출한 곳에서 상황에 맞는 메시지 표시
     updateNotificationStatus();
     return sub;
   } catch (error) {
     console.error("푸시 구독 중 오류 발생:", error);
     showSnackbar("푸시 알림 구독 중 오류가 발생했습니다.");
     updateNotificationStatus();
